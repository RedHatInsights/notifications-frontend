...
export const NotAuthorizedPage: React.FunctionComponent = () => {
    ...
    const userPreferences = <a href={ `${isBeta() ? '/preview' : ''}/user-preferences/notifications/` }> User Preferences</a>;
    const myUserAccess = <a href={ `${isBeta() ? '/preview' : ''}/settings/my-user-access?bundle=rhel` }> My User Access </a>;
    ...
    const description = React.useMemo(() => {
        if (serviceName === 'Notifications') {
            ...
            { <a href={ `${isBeta() ? '/preview' : ''}/user-preferences/notifications/` }> User Preferences</a>}.</span>;
        } else {
            return <span></span>;
        }
    }, [ serviceName, isBeta ]);
