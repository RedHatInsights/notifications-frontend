...
const transformPayload = <FROM, TO>(
  payload: FROM | undefined,
  status: number | undefined,
  adapter: (from: FROM, status) => TO
): TO | undefined => {
  if (payload) {
...
interface UseTransformQueryResponseType {
  <FROM, TO, USE_QUERY_RESPONSE_FROM extends ExpectedQueryResponse<FROM>>(
  ...
  <FROM, TO, USE_MUTATE_RESPONSE_FROM extends ExpectedMutateResponse<FROM>>(
    queryResponse: USE_MUTATE_RESPONSE_FROM,
    adapter: (from: FROM) => TO
