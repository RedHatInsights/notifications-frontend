...
const mockFacets = (
  bundles?: Array<Facet> | null | Promise<any>,
  applications?: Array<Facet> | null | Promise<any>
) => {
  if (bundles !== null) {
    fetchMock.get(
      '/api/notifications/v1.0/notifications/facets/bundles?includeApplications=false',
      (bundles as any)?.then
        ? (bundles as Promise<any>)
        : {
            body:
              bundles ??
              ([
  ...
  if (applications !== null) {
    fetchMock.get(
      ...
      (applications as any)?.then
        ? (applications as Promise<any>)
        : {
            body:
              applications ??
              ([
...
const mockBehaviorGroupsOfEventTypes = (
...
describe('src/pages/Notifications/List/Page', () => {
  ...
  describe('Clicking edit button of notification row', () => {
    ...
    it('and then cancel, removes done/cancel', async () => {
      ...
      await waitForAsyncEvents();
      await userEvent.click(getByLabelText(table, /cancel/i));
    ...
    it('and then save closes the modal and shows a notification if no changes are made', async () => {
      ...
      await waitForAsyncEvents();
      await userEvent.click(getByLabelText(table, /done/i));
