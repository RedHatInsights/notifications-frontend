/* eslint-disable jest-dom/prefer-in-document, testing-library/await-async-events, testing-library/no-unnecessary-act */
...
describe('src/pages/Integrations/Create/CreatePage', () => {
  it('Renders when is open', async () => {
    appWrapperSetup();
    render(
      <CreatePage initialIntegration={{}} onClose={fn()} isEdit={false} />,
      {
  ...
  it('If no IntegrationType is specified, it picks the first', async () => {
    ...
    await waitForAsyncEvents();
    const firstIntegrationTypeDisplayName =
      Config.integrations.types[Object.values(IntegrationType)[0]].name;
    expect(
      screen.getByDisplayValue(firstIntegrationTypeDisplayName)
    ).toBeTruthy();
    appWrapperCleanup();
  });
  ...
  it('After adding it shows a Modal with the option to disable if the integration was enabled', async () => {
    ...
    await waitForAsyncEvents();
    expect(
      fetchMock.called('/api/integrations/v1.0/endpoints/foobar/enable', {
        method: 'DELETE',
      })
    ).toBeTruthy();
    await waitForAsyncEvents();
